...
/**
	...
	*
	**/
void TSCADC::tsStepConfig(unsigned int stepSelect, NegativeRef adcNegativeRef, PositiveInput adcPositiveInp,
...
void TSCADC::clearBitsInReg(int addr, int ctr_tsc) {
    uint32_t current_value = in32(addr);
    current_value &= ~ctr_tsc;
    out32(addr, current_value);
}

void TSCADC::setBitsInReg(int addr, int ctr_tsc) {
    uint32_t current_value = in32(addr);
    current_value |= ctr_tsc;
    out32(addr, current_value);
}